...
int main(int argc, char **argv){
    ...
    if (little_endian==1){
        depth = (depth >> 24) | (depth >> 8 & 0x0000FF00) | (depth << 8 & 0x00FF0000) | (depth << 24);
	int q;
	for(q=0;q<num_ports;q++){
            int *this=&ports[q];
	    *this = (*this >> 24) | (*this >> 8 & 0x0000FF00) | (*this << 8 & 0x00FF0000) | (*this << 24);
	}
    }
    printf("num ports %d\n",num_ports);
    printf("depth %x\n",depth);
	...
	while (index < depth){
		...
		port++;
	      ...
	      else if (data[index]==48){//if "0" this ignores all other characters like "\n" (which should not be there anyway)
		offs ^= ports[port];
		//printf("%u is off\n", ports[port]);
		port++;
	      }
