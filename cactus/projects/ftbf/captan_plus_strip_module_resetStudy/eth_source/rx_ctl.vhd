...
entity rx_ctl is 
	port (
        ...
        rx_fifo_preemptive_reset: out STD_LOGIC;
...
signal monitor_rx_address: STD_LOGIC_VECTOR (63 downto 0);
signal monitor_rx_data: STD_LOGIC_VECTOR (63 downto 0);
...
begin
	if rising_edge(clock) then
		...
		--attempt to monitor address and data for rx_fifo_preemptive_reset
        if(monitor_rx_data(7 downto 0) = x"10" and monitor_rx_address = x"00000001FFFFFFFF") then
            rx_fifo_preemptive_reset <= '1';
        else 
            rx_fifo_preemptive_reset <= '0';
        end if;
	end if;
end process;
...
begin
	if clock'event and clock = '1' then
		if reset = '1' then
			...
			monitor_rx_address <= (others=>'0');
            monitor_rx_data <= (others=>'0');
		else
			if clken = '1' then
				-- Set default values for outputs, signals and variables
				data_fifo_wren_sig <= '0';
				...
				case Sreg0 is
					...
					when S3_S11 =>
						...
						--attempt to monitor address and data for rx_fifo_preemptive_reset 
                        if (q_w_counter = 1) then --look at address
                            monitor_rx_address <= user_rx_data_out & data_fifo_wdata_sig(63 downto 8);
                        elsif (q_w_counter = 2) then --look at data
                            monitor_rx_data <= user_rx_data_out & data_fifo_wdata_sig(63 downto 8);
                        end if;                                                        
