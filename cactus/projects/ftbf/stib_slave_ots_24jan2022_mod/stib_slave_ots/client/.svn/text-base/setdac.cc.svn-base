...
int main(int argc,char **argv) {
  ...
  msg->WaitClear(STRIP_DAC_SPI,0x80000000);              //  Wait for DAC SPI to finish
  //msg->Write(STRIP_DAC_INPUT,0xff810700);            //  Positive pulse, about 200 mV amplitude
  msg->Write(STRIP_DAC_INPUT,0x10900);            //  Positive pulse, about 200 mV amplitude
...
//
  ...
  for ( int i=0; i<12; i++ ) {
    cout << "DAC " << idac << " addr " << i << " = 0x" << hex << (msg->RxData(ind[i],1)&0xff) << dec << endl;
  }
  msg->Send();
