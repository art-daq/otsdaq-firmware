...
int main(int argc,char **argv) {
  ...
  msg->Clear();
  msg->Write(DATA_DESTINATION_IP,0xc0a88501);          //  Set data destination IP 192.168.133.1
  msg->Write(DATA_SOURCE_DESTINATION_PORT,0xbeefb798); //  Set data destination port 0xb798=47000
  //unsigned int sourceDestinationPort = (sourcePort << 16) + port;
  //msg->Write(DATA_SOURCE_DESTINATION_PORT,sourceDestinationPort); //  Set data destination port 0xb798=47000
  msg->Send(); msg->Clear();
  ...
  //return EXIT_SUCCESS;
  cout << "Analyzing pedestals for strips " << firstStrip << ".." << lastStrip << endl;
  TFile* outFile = TFile::Open("ModuleTest.root","RECREATE");
  outFile->cd();
  TH2F* hPedestal  = new TH2F("hPedestal" ,"Occupancy vs dvtp",641,-0.5,640.5,(dvtpBegin-dvtpEnd)/dvtpStep+1,dvtpEnd-dvtpStep/2.,dvtpBegin+dvtpStep/2.);
  TH2F* hThreshold = new TH2F("hThreshold","Occupancy vs dac" ,641,-0.5,640.5,(dacMax-dacMin)/dacStep+1,dacMin-dacStep/2.,dacMax+dacStep/2.);
  ...
  for(int analysis=0; analysis<=1; analysis++)
  {
    ...
    for (int i=0; i<8; i++ )
      msg->SlowControls(21,channelMask,DTHR+i,WRITE,defaultDthr[i]);  //  Set discriminator thresholds
    msg->Send(); msg->Clear();
    if(analysis == 0)
    {
    ...
    {
      cout << endl << "~~~~~~~~~~~~~ Analyzing strip " << strip << " ~~~~~~~~~~~~~~~~~~~~~~"<< endl;
      int iChip = strip/128;
      int chipId = iChip + 9;
        ...
        if(analysis == 0)
	{
	  ...
	  unsigned int dacWord = 0x00010000 | (((unsigned int)-vDac)&0x0fff);
    	...
    	msg->Write(STRIP_ANALYSIS_CSR,0x0000a000|(channel<<16)|(Stib::SetNumber(strip%128)<<4)|Stib::StripNumber(strip%128));
	...
	unsigned char ind[6];
    	...
    	msg->Send(); msg->Clear();
    	//cout << "Dvtp: " << dvtp  << " - Dvtn: " << dvtn << " = " << (int)(dvtp - dvtn) << endl;
    	if ( ( ierr = msg->Receive() ) < 0 ) {
    	  cout << "Strip " << strip << ", dvtn = " << dvtp << " - receive error = " << ierr << endl;
    	  --loop;
    	}
    	else 
	{
    	  ...
    	  double frac = num/den;
    	  cout << "Strip " << strip << ", dvtp = " << dvtp << ", vdac = " << vDac << ", frac = " << num << "/" << den << " = " << frac << endl;
          if(analysis == 0)
