...
int main(int argc,char **argv) {
  ...
  while(1) 
  {
    ...
    int j = -1;
    if ( (ntohl(daddr.sin_addr.s_addr)&0xff) == 20 ) j = 0;
    if ( (ntohl(daddr.sin_addr.s_addr)&0xff) == 36 ) j = 1;
    if ( (ntohl(daddr.sin_addr.s_addr)&0xff) == 132 ) j = 2;
    if ( j < 0 ) {
      if ( ntohs(daddr.sin_port) == 47101 ) j = 0;
      if ( ntohs(daddr.sin_port) == 48101 ) j = 1;
      if ( ntohs(daddr.sin_port) == 49101 ) j = 2;
    }
    if ( j < 0 ) {
      cout << "Port number " << ntohs(daddr.sin_port) << endl;
      continue;
    }
    cout << "Received from address " << hex << ntohl(daddr.sin_addr.s_addr) << ", j = " << j << endl;
    for ( int i=0; i<len/4; i++ ) {
      ...
      if ( type == 8 ) {
        ...
        cout << "Type: " << type << " DataType " << datatype 
             << " chan: " << chan << " chipid: " << chipid << " set: " << set << " strip: " << strip << " bco: " << bco << " istrip: " << istrip 
	     << " adc: " << adc 
	     << endl;
        if ( chan < 6 ) h_strip[chan][j]->Fill(istrip,1);
        nhit += 1;
      }
    }
    time(&t1);
    if ( t1-t0 > 5 ) {
      for ( int i=0; i<6; i++ ) {
        if ( h_strip[i][0]->GetEntries() > 0 || h_strip[i][1]->GetEntries() > 0 || h_strip[i][2]->GetEntries() > 0 ) {
          c1->cd(i+1);
          int j, jmax = 0;
          for ( j=1; j<3; j++ ) {
            if ( h_strip[i][j]->GetBinContent(h_strip[i][j]->GetMaximumBin()) >
                 h_strip[i][jmax]->GetBinContent(h_strip[i][jmax]->GetMaximumBin()) ) {
              jmax = j;
            }
          }
          if ( i < 4 ) {
            h_strip[i][jmax]->Draw();
            if ( jmax != 0 ) h_strip[i][0]->Draw("same");
            if ( jmax != 1 ) h_strip[i][1]->Draw("same");
            if ( jmax != 2 ) h_strip[i][2]->Draw("same");
          }
          else {
            h_strip[i][2]->Draw();
          }
