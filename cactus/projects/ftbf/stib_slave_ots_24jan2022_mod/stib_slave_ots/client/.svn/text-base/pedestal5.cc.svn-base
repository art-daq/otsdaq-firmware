...
int main(int argc,char **argv) {
  ...
  fout->cd();
  TH2F *h_ped = new TH2F("ped","occupancy vs dvtn",640,-0.5,639.5,16,-0.5,15.5);
  ...
  for ( int strip=first; strip<=last; strip++ ) {
    for ( int dvtn=0; dvtn<16; dvtn++ ) {
      ...
      for ( i=0; i<8; i++ ) {
        msg->SlowControls(21,chmask,DTHR+i,WRITE,16*i+16);  //  Set discriminator thresholds
      }
      ...
      msg->Write(STRIP_ANALYSIS_CSR,0x00009000|(chan<<16)|(Stib::SetNumber(strip)<<4)|Stib::StripNumber(strip));
      ...
      if ( ( ierr = msg->Receive() ) < 0 ) {
        ...
        for ( int i=0; i<5; i++ ) {
          ...
          double frac = num/den;
          cout << "Strip " << 128*i+strip << ", dvtn = " << dvtn << ", frac = " << num << "/" << den << " = " << frac << endl;
