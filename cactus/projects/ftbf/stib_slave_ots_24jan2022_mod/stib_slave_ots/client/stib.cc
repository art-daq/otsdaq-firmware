...
const char *Stib::_opname[] = {"undefined", "write",   "set",
                               "ack",       "read",    "reset",
...
Stib::Stib(int s, struct sockaddr *daddr, int dlen)
    : _socket(s), _daddr(daddr), _dlen(dlen), _pbuf(_txbuf) {
  _id = 0;
  _iseq = 1;
  *_txbuf = 0x00;
}
...
int Stib::WaitClear(unsigned int addr, unsigned int data,
...
int Stib::SlowControls(unsigned char chipid, unsigned char mask,
                       unsigned char addr, unsigned char inst) {
  unsigned int w;
  int ierr;
  int len = 0;
  ...
  if (inst == READ) {
    w = 0x80000000 | (len << 24) | (1 << (16 + mask)) | (mask << 13) |
        (inst << 10) | (addr << 5) | chipid;
  } else {
    w = 0x80000000 | (len << 24) | (mask << 16) | (inst << 10) | (addr << 5) |
        chipid;
  }
...
int Stib::MaskHotStrips(StripSet &s) {
  unsigned int mask[4];
  int hot = 0;

  //
  //  Write the mask set when a change chip id, channel or reach
  //  the end of the set.
  //
  StripSet::const_iterator i = s.begin();
  mask[0] = 0;
  mask[1] = 0;
  mask[2] = 0;
  mask[3] = 0;
  ...
  while (i != s.end()) {
    int chipid = i->ChipId();
    int chan = i->Channel();
    int strip = 127 - i->StripNumber();
    mask[strip / 32] |= (1 << (strip % 32));
    hs = *i++;
    if (i == s.end() || i->Channel() != hs.Channel() ||
        i->ChipId() != hs.ChipId()) {
      hot = 1;
      cout << "Channel " << hs.Channel() << " ChipId " << hs.ChipId()
           << " mask = " << hex << setw(8) << setfill('0') << mask[0] << " "
           << setw(8) << setfill('0') << mask[1] << " " << setw(8)
           << setfill('0') << mask[2] << " " << setw(8) << setfill('0')
           << mask[3] << dec << endl;
      SlowControls(chipid, 1 << chan, KILL, WRITE, mask);
      mask[0] = 0;
      mask[1] = 0;
      mask[2] = 0;
      mask[3] = 0;
