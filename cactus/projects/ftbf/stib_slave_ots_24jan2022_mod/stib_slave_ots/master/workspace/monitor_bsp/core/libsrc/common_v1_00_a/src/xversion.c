 ...
 * as a string.
 *
 * @param    InstancePtr points to the version to unpack the packed version
 *into.
...
static int IsVersionStringValid(char *StringPtr) {
   ...
   */
  if ((StringPtr[XVE_MAJOR_CHAR] < '0') || (StringPtr[XVE_MAJOR_CHAR] > '9') ||
      ...
      (StringPtr[XVE_COMP_CHAR] < 'a') || (StringPtr[XVE_COMP_CHAR] > 'z')) {
    return FALSE;
  }
