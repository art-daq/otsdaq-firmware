...
#define CAPTAN_VIPIC_CSR              (*(volatile unsigned int *)(0xc9000000))
#define CAPTAN_VIPIC_CLOCK            (*(volatile unsigned int *)(0xc9000004))
#define CAPTAN_VIPIC_SYNC             (*(volatile unsigned int *)(0xc9000010))
#define CAPTAN_VIPIC_IDELAY           (*(volatile unsigned int *)(0xc9000014))
#define CAPTAN_VIPIC_TS_LOW           (*(volatile unsigned int *)(0xc9000040))
#define CAPTAN_VIPIC_TS_HIGH          (*(volatile unsigned int *)(0xc9000044))
#define CAPTAN_VIPIC_DCM              (*(volatile unsigned int *)(0xc9000048))
#define CAPTAN_VIPIC_FIFO_STATUS      (*(volatile unsigned int *)(0xc9000024))
#define CAPTAN_VIPIC_FIFO_DATA        (*(volatile unsigned int *)(0xc9000028))
#define CAPTAN_VIPIC_WSET             ((volatile unsigned int *)(0xc9001000))
#define CAPTAN_VIPIC_WRESET           ((volatile unsigned int *)(0xc9001400))
#define CAPTAN_VIPIC_WCAL             ((volatile unsigned int *)(0xc9002000))
...
int cmd_ts(const struct s_cmd *c) {
  int ierr = 1;
  if ( narg == 1 ) {
    _puts("\r\n TS = ");
    _print_hex_long(CAPTAN_VIPIC_TS_HIGH);
    _print_hex_long(CAPTAN_VIPIC_TS_LOW);
    ierr = 0;
  }
  else if ( narg == 2 ) {
    if ( _strcmp(args[1],"clear") == 0 ) {
      CAPTAN_VIPIC_CSR |= 0x00400000;
      CAPTAN_VIPIC_CSR &= ~0x00400000;
      ierr = 0;
    }
  }
...
int cmd_tsclk(const struct s_cmd *c) {
  int i, ierr = 1;
  unsigned char n, d;
  int t;
  float f;
  ...
  if ( narg == 1 ) {
    ierr = 0;
    CAPTAN_VIPIC_DCM = 0x00500000;
    n = ((CAPTAN_VIPIC_DCM)&0x1f) + 1;
    CAPTAN_VIPIC_DCM = 0x00520000;
    d = ((CAPTAN_VIPIC_DCM)&0x1f) + 1;
    if ( (CAPTAN_VIPIC_CSR&0x00010000) ) {
//      t = 25;       //  This is for a 40 MHz external clock
    ...
    _print_float(f);
    _puts(" MHz -");
    if ( ! (CAPTAN_VIPIC_CSR&0x40000000) ) _puts(" not");
    _puts(" locked");
  }
  else if ( narg > 1 ) {
    for ( i=1; i<narg; i++ ) {
      if ( _strcmp(args[i],"reset") == 0 ) {
        CAPTAN_VIPIC_CSR |= 0x80000000;
        for ( i=0; i<1000; i++ );
        CAPTAN_VIPIC_CSR &= ~0x80000000;
        continue;
      }
      else if ( _strcmp(args[i],"int") == 0 ) {
        CAPTAN_VIPIC_CSR &= ~0x00010000;
        continue;
      }
        ...
        if ( *p == '/' ) {
          ...
          if ( _isdigit(*p) ) {
            d = _atoi(p);
            CAPTAN_VIPIC_DCM = 0x80500000|(n-1);
            while ( (CAPTAN_VIPIC_DCM&0x80000000) );
            CAPTAN_VIPIC_DCM = 0x80520000|(d-1);
            while ( (CAPTAN_VIPIC_DCM&0x80000000) );
            continue;
          }
          else {
...
void print_version() {
  _puts("\r\nMaster STIB/VIPIC DUT monitor rev 0.20\r\nBuilt ");
  _puts(__DATE__);
  _putch(' ');
  _puts(__TIME__);
  _puts("\r\n");
...
int cmd_restart(const struct s_cmd *c) {
  unsigned int w = CAPTAN_VIPIC_CLOCK;
  CAPTAN_VIPIC_CLOCK = 0xa000000f | (w&0x00f00000);
  return 0;
}

int cmd_clean(const struct s_cmd *c) {
  unsigned int w = CAPTAN_VIPIC_CLOCK;
  CAPTAN_VIPIC_CLOCK = 0x8010000f | (w&0x00f00000);
  return 0;
}

int cmd_read(const struct s_cmd *c) {
  int n = 1;
  if ( narg > 1 ) {
    n = _atoi(args[1]);
  }
  CAPTAN_VIPIC_CLOCK = 0x80030000 | (n<<8) | (5*n+5);
  return 0;
}

int cmd_go(const struct s_cmd *c) {
  int n = 1;
  if ( narg > 1 ) {
    n = _atoi(args[1]);
  }
  CAPTAN_VIPIC_CLOCK = 0x00730010 | (n<<8);
  return 0;
}

int cmd_set(const struct s_cmd *c) {
  int i, p, j, b;
  unsigned int w = 0;
  if ( narg > 1 ) {
    if ( _strcmp(args[1],"all") == 0 ) {
      for ( i=0; i<128; i++ ) {
        CAPTAN_VIPIC_WSET[i] = 0xffffffff;
      }
      w = CAPTAN_VIPIC_CLOCK;
      CAPTAN_VIPIC_CLOCK = 0x01000000 | (w&0x000fffff);
      return 0;
    }
  }
  for ( i=0; i<128; i++ ) {
    CAPTAN_VIPIC_WSET[i] = 0;
  }

  for ( i=1; i<narg; i++ ) {
    p = _atoi(args[i]);
    if ( p < 0 || p > 4095 ) return 1;
    j = p>>5;
    b = p&0x1f;
    CAPTAN_VIPIC_WSET[j] |= (1<<b);
  }
  w = CAPTAN_VIPIC_CLOCK;
  CAPTAN_VIPIC_CLOCK = 0x01000000 | (w&0x01fffff);
  return 0;
}

int cmd_cal(const struct s_cmd *c) {
  unsigned int w;
  int i;
  if ( _strcmp(args[1],"on") == 0 ) {
    w = 0x0fff0fff;
  }
  else if ( _strcmp(args[1],"off") == 0 ) {
    w = 0;
  }
  else {
    return 1;
  ...
  for ( i=0; i<512; i++ ) {
    CAPTAN_VIPIC_WCAL[i] = w;
  }
  w = CAPTAN_VIPIC_CLOCK;
  CAPTAN_VIPIC_CLOCK = 0x04000000 | (w&0x01fffff);  // shift D
  while ( (CAPTAN_VIPIC_CLOCK&0x04000000) );
  CAPTAN_VIPIC_CLOCK = 0x08000000 | (w&0x01fffff);  // load
  while ( (CAPTAN_VIPIC_CLOCK&0x08000000) );

  return 0;
}

int cmd_res(const struct s_cmd *c) {
  int i, p, j, b;
  unsigned int w;
  if ( narg > 1 ) {
    if ( _strcmp(args[1],"all") == 0 ) {
      for ( i=0; i<128; i++ ) {
        CAPTAN_VIPIC_WRESET[i] = 0xffffffff;
      }
      w = CAPTAN_VIPIC_CLOCK;
      CAPTAN_VIPIC_CLOCK = 0x02000000 | (w&0x000fffff);
      return 0;
    }
  }
  for ( i=0; i<128; i++ ) {
    CAPTAN_VIPIC_WRESET[i] = 0;
  }
  for ( i=1; i<narg; i++ ) {
    p = _atoi(args[i]);
    if ( p < 0 || p > 4095 ) return 1;
    j = p>>5;
    b = p&0x1f;
    CAPTAN_VIPIC_WRESET[j] |= (1<<b);
  }
  w = CAPTAN_VIPIC_CLOCK;
  CAPTAN_VIPIC_CLOCK = 0x02000000 | (w&0x01fffff);
  return 0;
}

int cmd_tsread(const struct s_cmd *c) {
  int n = 1;
  if ( narg > 1 ) {
    n = _atoi(args[1]);
  }
  CAPTAN_VIPIC_CLOCK = 0xc0030000 | (n<<8) | (5*n+5);
  return 0;
}
...
int cmd_spy(const struct s_cmd *c) {
  ...
  if ( narg > 1 ) {
    if ( _strcmp(args[1],"clear") == 0 ) {
      CAPTAN_VIPIC_FIFO_STATUS |= 0x80000000;
      CAPTAN_VIPIC_FIFO_STATUS &= ~0x80000000;
      return 0;
    }
    return 1;
  }
  if ( (CAPTAN_VIPIC_FIFO_STATUS&0x10000000) ) {
    _puts("\r\nempty");
    return 0;
  }
  while ( ! (CAPTAN_VIPIC_FIFO_STATUS&0x10000000) && ! (UART_SR&0x01) ) {
    w = CAPTAN_VIPIC_FIFO_DATA;
    _puts("\r\n  ");
    _print_hex_long(w);
    _puts(" = ");
    v = (w>>16)&0x0f00;
...
int cmd_idelay(const struct s_cmd *c) {
  int chan, bad, i;
  unsigned int idelay[2] = { 0, 0 };
  if ( _strcmp(args[1],"sync") == 0 ) {
    _puts("\r\nidelay = ");
    for ( chan=0; chan<16; chan++ ) {
      ...
      do {
        for ( i=0; i<8; i++ ) {
          if ( ! (CAPTAN_VIPIC_SYNC&(1<<chan)) ) bad = 1;
        }
        CAPTAN_VIPIC_IDELAY = 0x90000000 | chan;
      }
      while ( bad && j++ < 64 );
      if ( ! bad ) {
        _print_int((CAPTAN_VIPIC_IDELAY>>16)&0xff);
      }
      else {
        _puts("XXX");
      }
    ...
    for ( i=0; i<64; i++ ) {
      CAPTAN_VIPIC_IDELAY = 0x90000000 | chan;
      id = (CAPTAN_VIPIC_IDELAY>>16)&0x3f;
      for ( j=0; j<8; j++ ) {
        if ( ! (CAPTAN_VIPIC_SYNC&(1<<chan)) ) {
          idelay[id/32] |= (1<<(id%32));
        }
      }
    }
    _puts("\r\n");
    for ( i=0; i<64; i++ ) {
      if ( idelay[i/32]&(1<<(i%32)) ) {
        _putch('X');
      }
      else {
        _putch('_');
