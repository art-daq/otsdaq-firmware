...
#define XIN_ISR (*(volatile unsigned int *)0x80000030)  /**< Intr Status Register      - R */
#define XIN_IPR (*(volatile unsigned int *)0x80000034)  /**< Intr Pending Register     - R */
#define XIN_IER (*(volatile unsigned int *)0x80000038)  /**< Intr Enable Register      - W */
#define XIN_IAR (*(volatile unsigned int *)0x8000003C)  /**< Intr Acknowledge Register - W */
...
#define CAPTAN_PSI46_CSR              (*(volatile unsigned int *)(0xc6000000))
#define CAPTAN_PSI46_RESET            (*(volatile unsigned int *)(0xc6000004))
#define CAPTAN_PSI46_I2C              (*(volatile unsigned int *)(0xc6000008))
#define CAPTAN_PSI46_I2C_DATA         (*(volatile unsigned int *)(0xc600000c))
#define CAPTAN_PSI46_CALTRIGTBM       (*(volatile unsigned int *)(0xc6000010))
#define CAPTAN_PSI46_STATUS           (*(volatile unsigned int *)(0xc6000018))
#define CAPTAN_PSI46_INPUT            (*(volatile unsigned int *)(0xc6000020))
#define CAPTAN_PSI46_SPY_FIFO         (*(volatile unsigned int *)(0xc6000030))
#define CAPTAN_PSI46_BCO_LOW          (*(volatile unsigned int *)(0xc6000040))
#define CAPTAN_PSI46_BCO_HIGH         (*(volatile unsigned int *)(0xc6000044))
#define CAPTAN_PSI46_BCOCLK           (*(volatile unsigned int *)(0xc6000048))

#define CAPTAN_MASTER_CSR             (*(volatile unsigned int *)(0xc5000000))
#define CAPTAN_MASTER_BDCLKFREQ       (*(volatile unsigned int *)(0xc5000004))
#define CAPTAN_MASTER_COUNT           (*(volatile unsigned int *)(0xc5000008))
...
const char *regname[] = { "null", "Vdig", "Vana", "Vsf", "Vcomp", "Vleak", "VrgPr", "VwllPr",
                          "VrgSh", "VwllSh", "VHldDel", "Vtrim", "VcThr", "VIbias_bus", "VIbias_sf", "VOffsetOp",
                          "VbiasOp", "VOffsetRO", "VIon", "VIbias_PH", "VIbias_DAC", "VIbias_ROC", "VIColOr",
                          "Vnpix", "VsumCol", "Vcal", "CalDel", "CtrlReg", "WBC", "NULL" };
const int nreg = 30;
...
//
  if ( narg == 1 ) {
    ...
    if ( ( ip = _parse_ip(args[1]) ) == 0 ) return 1;
    if ( ( nm = _parse_ip(args[2]) ) == 0 ) return 1;
    if ( ( gw = _parse_ip(args[3]) ) == 0 ) return 1;
    CAPTAN_IP_ADDR = ip;
    CAPTAN_NETMASK = nm;
    CAPTAN_GATEWAY = gw;
  }
...
int cmd_send(const struct s_cmd *c) {
  ...
  CAPTAN_SENDPKT_CSR = 0x00000000;   //  We need a few clock cycles to let the clear propagate
...
} dhcp = { 0x01, 0x01, 0x06, 0x00, { 0x39, 0x03, 0xf3, 0x26 },
...
int send_dhcp_request() {
  static struct s_dhcpdisc {
    ...
    unsigned char fin;
                      ...
                      { 53, 1 }, { 1, 0 },
                      { 54, 7 }, { 1, 28, 3, 15, 61, 42, 26, 0, 0, 0 }, 0xff };
  ...
  *((unsigned char *)dhcp.siaddr) = 0;
  ...
  for ( i=0; i<196; i++ ) CAPTAN_SENDPKT_CSR = 0x40000000;
  for ( i=0; i<21; i++ ) {
  ...
  for ( i=0; i<10000; i++ );    // Keep the ip address asserted for a while
...
int cmd_dhcp(const struct s_cmd *c) {
...
int cmd_bco(const struct s_cmd *c) {
  ...
  if ( narg == 1 ) {
    _puts("\r\n BCO = ");
    _print_hex_long(CAPTAN_PSI46_BCO_HIGH);
    _print_hex_long(CAPTAN_PSI46_BCO_LOW);
    ierr = 0;
  }
  else if ( narg == 2 ) {
    if ( _strcmp(args[1],"clear") == 0 ) {
      CAPTAN_PSI46_CSR |= 0x00400000;
      CAPTAN_PSI46_CSR &= ~0x00400000;
      ierr = 0;
    }
  }
...
int cmd_bcoclk(const struct s_cmd *c) {
  ...
  if ( narg == 1 ) {
    ierr = 0;
    CAPTAN_PSI46_BCOCLK = 0x00500000;
    n = ((CAPTAN_PSI46_BCOCLK)&0x1f) + 1;
    CAPTAN_PSI46_BCOCLK = 0x00520000;
    d = ((CAPTAN_PSI46_BCOCLK)&0x1f) + 1;
    if ( (CAPTAN_PSI46_CSR&0x00010000) ) {
//      t = 25;       //  This is for a 40 MHz external clock
    ...
    _print_float(f);
    _puts(" MHz -");
    if ( ! (CAPTAN_PSI46_CSR&0x40000000) ) _puts(" not");
    _puts(" locked");
  }
  else if ( narg > 1 ) {
    for ( i=1; i<narg; i++ ) {
      if ( _strcmp(args[i],"reset") == 0 ) {
        CAPTAN_PSI46_CSR |= 0x80000000;
        for ( i=0; i<1000; i++ );
        CAPTAN_PSI46_CSR &= ~0x80000000;
        continue;
      }
      else if ( _strcmp(args[i],"int") == 0 ) {
        CAPTAN_PSI46_CSR &= ~0x00010000;
        continue;
      }
        ...
        n = _atoi(args[i]);
        p = args[i];
        while ( *p != '\0' && _isdigit(*p) ) p++;
        if ( *p == '/' ) {
          p++;
          if ( _isdigit(*p) ) {
            d = _atoi(p);
            CAPTAN_PSI46_BCOCLK = 0x80500000|(n-1);
            while ( (CAPTAN_PSI46_BCOCLK&0x80000000) );
            CAPTAN_PSI46_BCOCLK = 0x80520000|(d-1);
            while ( (CAPTAN_PSI46_BCOCLK&0x80000000) );
            continue;
          }
          else {
...
char def_regnum[] =          {  1,   2,  3,  4,  7,  9,  10, 11, 12, 13, 14, 15, 17,  18, 19, 20, 22, 25,   26, 253, 254 };
unsigned char def_regval[] = { 15, 140, 40, 12, 60, 60, 117, 29, 50,  5,  6, 60, 150, 45, 50, 70, 10, 200, 112, 0, 100 };

void pixel_i2c_dac(unsigned char id, unsigned char addr, unsigned char val) {
  if ( (CAPTAN_PSI46_I2C&0x80000000) ) {
    _puts("\r\npixel_i2c_dac() - busy");
    return;
  }
  CAPTAN_PSI46_I2C_DATA = 0x00080000 | ((id&0x0f)<<20) | (addr<<8) | val;
  CAPTAN_PSI46_I2C = 0x80000003 | ((id&0xf0)<<12);
}

int cmd_prog(const struct s_cmd *c) {
  int i, j;
  for ( i=0; i<sizeof(def_regnum); i++ ) {
    if ( def_regnum[i] < 0 ) {
      j = nreg+def_regnum[i];
    }
    else {
    ...
    _puts(regname[j]);
    _puts(" (");
    _print_int((unsigned char)def_regnum[i]);
    _puts(") <-- ");
    _print_int(def_regval[i]);
    pixel_i2c_dac(0x1f,def_regnum[i],def_regval[i]);
  ...
  CAPTAN_PSI46_I2C_DATA = 0x04004080;    // Enable dcol 0
  CAPTAN_PSI46_I2C = 0x80010004;

  _puts("\r\nCalibrate dcol 0, pixel 0");
  if ( (CAPTAN_PSI46_I2C&0x80000000) ) {
    _puts("\r\ncmd_prog(2) - busy");
    return 0;
  }
  CAPTAN_PSI46_I2C_DATA = 0x02000001;    // Calibrate dcol 0 pixel 0
  CAPTAN_PSI46_I2C = 0x80010004;

  return 0;
}

int cmd_daci2c(const struct s_cmd *c) {
  ...
  CAPTAN_PSI46_I2C_DATA = 0x00080000 | (addr<<8) | val;
  CAPTAN_PSI46_I2C = 0x80010003;
  return 0;
}

int cmd_rdac(const struct s_cmd *c) {
  int i, j;
  unsigned int word;

  for ( i=0; i<5 && (CAPTAN_PSI46_STATUS&1) == 0; i++ ) {
    CAPTAN_PSI46_CALTRIGTBM = 0x20881010;
    while ( (CAPTAN_PSI46_CALTRIGTBM&0x20000000) );
  }
  if ( (CAPTAN_PSI46_STATUS&1) == 0 ) {
    _puts("\r\nDid not sync.");
    return 0;
  }
  do {
  ...
  do {
    ...
    while ( (CAPTAN_PSI46_CALTRIGTBM&0x20000000) );
    for ( j=0; j<100; j++ );
    word = (word<<1)|(CAPTAN_PSI46_INPUT&1);
  }
  while ( (CAPTAN_PSI46_INPUT&2) == 0 );

  _puts("\r\nDAC ");
  _print_hex_byte((word>>8)&0xff);
  _puts(" = ");
  _print_int(word&0xff);
  return 0;
}

int cmd_inject(const struct s_cmd *c) {
  int col;
  int row;
  int how;
  col = _atoi(args[1]); 
  row = _atoi(args[2]);
  how = 1;
  if ( narg > 3 ) how = _atoi(args[3]);
  if ( how != 0 ) {
    CAPTAN_PSI46_I2C_DATA = 0x04000080 | (col<<16) | (row<<8);
  }
  else {
    CAPTAN_PSI46_I2C_DATA = 0x04000000 | (col<<16) | (row<<8);
  }
  CAPTAN_PSI46_I2C = 0x80010004;
  _puts("\r\nEnable col ");
  _print_hex_byte(col);
  _puts(" row ");
  _print_hex_byte(row);
  while ( (CAPTAN_PSI46_I2C&0x80000000) );

  CAPTAN_PSI46_I2C_DATA = 0x02000000 | (col<<16) | (row<<8) | how;
  CAPTAN_PSI46_I2C = 0x80010004;
  while ( (CAPTAN_PSI46_I2C&0x80000000) );
  _puts(" cal ");
  _print_hex_byte(how);
...
void print_version() {
  _puts("\r\nMaster STIB/PSI46 DUT monitor rev 0.1\r\nBuilt ");
  _puts(__DATE__);
  _putch(' ');
  _puts(__TIME__);
  _puts("\r\n");
...
int cmd_spy(const struct s_cmd *c) {
  unsigned int w;
  int i, n, dcol, pixl, adc, printed;
  unsigned int sr;
  char b;
  if ( narg > 1 ) {
    i = _atoi(args[1]);
    if (  i < 0 || i > 3 ) return 1;
    w = CAPTAN_PSI46_SPY_FIFO;
    w &= ~0x0f000000;
    w |= i<<24;
    CAPTAN_PSI46_SPY_FIFO = w;
    return 0;
  }
  n = 0;
  sr = 0;
  ...
  do {
    ...
    if ( (w&0x00000100) == 0 ) {
      ...
      if ( (sr&0x7f8) == 0x7f8 ) {
        ...
        dcol = 6*((sr>>21)&0x07)+((sr>>18)&0x07);
        pixl = 36*((sr>>15)&0x07)+6*((sr>>12)&0x07)+((sr>>9)&0x07);
        b = (pixl&1)?'R':'L';
        pixl = 80-pixl/2;
        adc = 16*((sr>>5)&0x0f)+(sr&0x0f);
        _print_int(dcol);
        _putch(',');
        _print_int(pixl);
        _putch(b);
  ...
  while ( (w&0x00000100) == 0 && ! (UART_SR&0x01) );
  if ( (UART_SR&0x01) ) {
    _getch();
  }
  return 0;
...
int cmd_halt(const struct s_cmd *c) {
  CAPTAN_MASTER_CSR |= 4;
  return 0;
}

int cmd_start(const struct s_cmd *c) {
  CAPTAN_MASTER_CSR |= 8;
  return 0;
}

int cmd_count(const struct s_cmd *c) {
  unsigned int w = CAPTAN_MASTER_COUNT;
  _puts("\r\ntriggers = ");
  _print_int((w>>16)&0xff);
  _puts("\r\nstart = ");
  _print_int(w&0xff);
  _puts("\r\nhalt = ");
  _print_int((w>>8)&0xff);
...
int cmd_bdclk(const struct s_cmd *c) {
  int bits;
  if ( narg == 1 ) {
    unsigned int f = CAPTAN_MASTER_BDCLKFREQ;
    _puts("\r\nGenerated clock freq = ");
    _print_int(f/1000000);
    _putch('.');
    _print_int(f%1000000);
    _puts(" MHz");
  }
  else if ( narg == 2 ) {
    if ( _strcmp(args[1],"reset") == 0 ) {
      CAPTAN_MASTER_CSR |= 0x80000000;
      CAPTAN_MASTER_CSR &= 0x7fffffff;
    }
    else if ( _strcmp(args[1],"int") == 0 ) {
      bits = 1;
    }
    else if ( _strcmp(args[1],"int/2") == 0 ) {
      bits = 3;
    }
    else if ( _strcmp(args[1],"ext") == 0 ) {
      bits = 0;
    }
    else if ( _strcmp(args[1],"ext*2") == 0 ) {
      bits = 2;
    }
    else {
      return 1;
    }
    CAPTAN_MASTER_CSR = 0x80000000 | (bits<<28);
    CAPTAN_MASTER_CSR = (bits<<28);
  }
  return 0;
}

struct s_cmd cmds[] = { { "peek", 1, cmd_peek },
                        { "poke", 2, cmd_poke },
                        { "rx", 0, cmd_rx },
                        { "tx", 0, cmd_tx },
                        { "ip", 0, cmd_ip },
                        ...
                        { "bcoclk", 0, cmd_bcoclk },
                        { "program", 0, cmd_prog },
                        { "vdig", 0x0102, cmd_daci2c },
                        { "vana", 0x0202, cmd_daci2c },
                        { "vsh", 0x0302, cmd_daci2c },
                        { "vcomp", 0x0402, cmd_daci2c },
                        { "vwllpr", 0x0702, cmd_daci2c },
                        { "vwllsh", 0x0902, cmd_daci2c },
                        { "vhlddel", 0x0a02, cmd_daci2c },
                        { "vtrim", 0x0b02, cmd_daci2c },
                        { "vthrcomp", 0x0c02, cmd_daci2c },
                        { "vibias_bus", 0x0d02, cmd_daci2c },
                        { "vbias_sf", 0x0e02, cmd_daci2c },
                        { "voffsetop", 0x0f02, cmd_daci2c },
                        { "phoffset", 0x1102, cmd_daci2c },
                        { "vion", 0x1202, cmd_daci2c },
                        { "vcomp_adc", 0x1302, cmd_daci2c },
                        { "phscale", 0x1402, cmd_daci2c },
                        { "vicolor", 0x1602, cmd_daci2c },
                        { "vcal", 0x1902, cmd_daci2c },
                        { "caldel", 0x1a02, cmd_daci2c },
                        { "ctrlreg", 0xfd02, cmd_daci2c },
                        { "wbc", 0xfe02, cmd_daci2c },
                        { "rdac", 0, cmd_rdac },
                        { "spy", 0, cmd_spy },
                        { "inject", 3, cmd_inject },
                        { "halt", 0, cmd_halt },
...
void handler(void) {
  ...
  static const char *cmdname[] = { "???", "write  ", "set    ", "recv   ", "read   ", "reset  ", "default", "waitclr", "waitset" };
  ...
  if ( ((CAPTAN_UDP_PORTS>>16)&0xffff) == 123 ) {
    ...
    _print_ip_addr(t);
    _puts(" ");
    for ( i=0; i<220; i++ ) b = CAPTAN_UDP_FIFO_DATA&0xff;
    b = CAPTAN_UDP_FIFO_DATA&0xff;
    if ( b != 53 ) {
      _print_hex_byte(b);
      _puts(" != 53 ");
        ...
        if ( cmd == 1 || cmd == 2 || cmd == 5 || cmd == 7 || cmd == 8 ) {
          ...
          if ( cmd == 1 ) {
            ...
            *((volatile unsigned int *)addr) &= data;
            ...
            do {
              i += 1;
              if ( cmd == 7 && (*((volatile unsigned int *)addr)&data) == 0 ) break;
              if ( cmd == 8 && (*((volatile unsigned int *)addr)&data) != 0 ) break;
              if ( (i%100000) == 0 ) _putch('.');
            }
            while ( i < count );
            if ( (msglog&LOG_ETHIO) ) {
              _puts("  loop=");
    ...
    CAPTAN_SENDPKT_CSR = 0x40000000;
    CAPTAN_SENDPKT_CSR = 0x40000000;
    CAPTAN_SENDPKT_CSR = 0x40000000;   //  Extra padding in case the payload isn't big enough
...
int main() {
  ...
  if ( CAPTAN_GBE_CSR&DHCP_BIT ) {
    ...
    do {
      ...
      send_dhcp_request();
      j = 0;
      while ( j < 20000000 && dhcp_done == 0 ) j++;
    }
  ...
  _print_ip_addr(CAPTAN_GATEWAY);
  for ( i=0; i<1000; i++ );
  if ( dhcp_ntpaddr != 0 ) send_ntp(dhcp_ntpaddr);
