...
const char *regname[] = {
    "null",    "Vdig",       "Vana",       "Vsf",        "Vcomp",   "Vleak",
    "VrgPr",   "VwllPr",     "VrgSh",      "VwllSh",     "VHldDel", "Vtrim",
    "VcThr",   "VIbias_bus", "VIbias_sf",  "VOffsetOp",  "VbiasOp", "VOffsetRO",
    "VIon",    "VIbias_PH",  "VIbias_DAC", "VIbias_ROC", "VIColOr", "Vnpix",
    "VsumCol", "Vcal",       "CalDel",     "CtrlReg",    "WBC",     "NULL"};
const int nreg = 30;
...
int cmd_bcoclk(const struct s_cmd *c) {
  ...
  if (narg == 1) {
    ...
    _print_float(f);
    _puts(" MHz -");
    if (!(CAPTAN_PSI46_CSR & 0x40000000))
      _puts(" not");
    _puts(" locked");
  } else if (narg > 1) {
    for (i = 1; i < narg; i++) {
...
char def_regnum[] = {1,  2,  3,  4,  7,  9,  10, 11, 12,  13, 14,
                     15, 17, 18, 19, 20, 22, 25, 26, 253, 254};
unsigned char def_regval[] = {15, 140, 40, 12, 60, 60, 117, 29,  50, 5,  6,
                              60, 150, 45, 50, 70, 10, 200, 112, 0,  100};
  ...
  do {
    ...
    if ((w & 0x00000100) == 0) {
      ...
      if ((sr & 0x7f8) == 0x7f8) {
        ...
        pixl = 36 * ((sr >> 15) & 0x07) + 6 * ((sr >> 12) & 0x07) +
               ((sr >> 9) & 0x07);
        b = (pixl & 1) ? 'R' : 'L';
        pixl = 80 - pixl / 2;
