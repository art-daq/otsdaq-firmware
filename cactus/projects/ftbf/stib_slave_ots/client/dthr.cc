...
double ferf(double *x,double *p) {
  return p[2]+(1-p[2])*(0.5+0.5*TMath::Erf((x[0]-p[0])/p[1]/sqrt(2)));
}
...
int main(int argc,char **argv) {
  ...
  if ( argc < 5 ) {
    cerr << "Usage: " << argv[0] << " <addr> <chan> <first> <last> [dcr]" << endl;
    exit(1);
  }
  int dcr = 16;
  if ( argc == 6 ) dcr = atoi(argv[5]);
  ...
  cout << "Analyzing pedestals for channel " << chan << " strips " << first << ".." << last << endl;
  TFile *fout = new TFile("dthrscan.root","RECREATE");
  fout->cd();
  int ndthr = 5;
  int dthrmin = 20;
  int dthrmax = 24;
  TH2F *h_mu = new TH2F("mu",NULL,640,-0.5,639.5,ndthr,dthrmin-0.5,dthrmax+0.5);
  TH2F *h_sig = new TH2F("sig",NULL,640,-0.5,639.5,ndthr,dthrmin-0.5,dthrmax+0.5);
  
  Stib *msg = new Stib(s,(struct sockaddr *)&daddr,sizeof(daddr));
  msg->Clear();
  msg->Write(STRIP_CSR,0x00000300);                      //  Reset CSR, enable only channels 0,1
//  msg->Write(STRIP_BCO_DCM,0x80500002);                //  Set numerator to 6
//  msg->WaitClear(STRIP_BCO_DCM,0x80000000);            //  Wait DCM write to finish
//  msg->Write(STRIP_BCO_DCM,0x80520004);                //  Set denominator to 4
//  msg->WaitClear(STRIP_BCO_DCM,0x80000000);            //  Wait DCM write to finish
//  msg->Write(STRIP_CSR,0x80000000|(chmask<<8));        //  Reset the DCM  - this bit is not self-clearing.
  msg->Write(STRIP_CSR,0x0a000000|(chmask<<8));
  msg->WaitClear(STRIP_CSR,0x80000000,1024);             //  Wait for the reset to complete.
  msg->Send();
  ...
  for ( int strip=first; strip<=last; strip++ ) {
    ...
    for ( int dthr=dthrmin; dthr<=dthrmax; dthr++ ) {
      ...
      do {
        ...
        msg->SlowControls(chipid,chmask,DTHR,WRITE,dthr);  //  Set discriminator threshold
        ...
        msg->Write(STRIP_ANALYSIS_CSR,0x0000a000|(chan<<16)|(Stib::SetNumber(strip)<<4)|Stib::StripNumber(strip));
        ...
        msg->Send();
        if ( ( ierr = msg->Receive() ) < 0 ) {
          cout << "Strip " << strip << ", vdac = " << vdac << " - receive error = " << ierr << endl;
        }
        else {
          unsigned int den = msg->RxData(ind[0],1);
          cout << "Denominator = 0x" << hex << den << dec << endl;
          if ( den == 0xffffffff ) continue;
          ...
          if ( efrac == 0 ) efrac = 0.001;
          cout << "Strip " << strip << ", vdac = " << vdac << ", frac = " << num << "/" << den << " = " << frac << endl;
          int npt = g->GetN();
          g->SetPoint(npt,vdac,frac);
          if ( mu < 1 && frac > 0.5 ) mu = vdac;
      ...
      h_sig->SetBinContent(strip+1,dthr-dthrmin+1,func->GetParameter(1));
