...
int main(int argc,char **argv) {
  ...
  msg->Clear();
  msg->Write(STRIP_CSR,0x00000000|(chmask<<8));        //  Reset CSR - reset trigger counter, internal clock source
  ...
  msg->WaitClear(STRIP_BCO_DCM,0x80000000);            //  Wait DCM write to finish - BCO frequency is now 15.000 MHz.
  ...
  msg->Write(DATA_DESTINATION_IP,0xc0a80166);          //  Destination IP address 192.168.1.102
  msg->Write(DATA_SOURCE_DESTINATION_PORT,0xbeefb798); //  Source and destination ports - dst port = 47000
...
//
  ...
  msg->SlowControls(21,chmask,DCR,WRITE,16);           // dcr write 16 - mod 256, high gain
...
// 
    ...
    cout << "Channel 0, chip 3: kill " << hex << mask[0] << " " << mask[1] << " " << mask[2] << " " << mask[3] << endl;
...
//
  ...
  msg->Write(STRIP_CSR,0x0f000000|(chmask<<8));        //  Turn on data readout in CSR but don't veto untriggered events.
  msg->Write(STRIP_TRIG_CSR,0x00000004);               //  BCO offset
  msg->Write(STRIP_TRIG_UNBIASED,0x1002805c);          //  Configure unbiased trigger
  msg->Write(STRIP_TRIG_INPUT_0,0x1f440000);           //  FSSR2 GOTHIT trigger input channel 0
  msg->Write(STRIP_TRIG_INPUT_2,0x2f440000);           //  FSSR2 GOTHIT trigger input channel 5
  msg->Write(STRIP_TRIG_INPUT_3,0x30301000);           //  External and unbiased trigger input
