...
int main(int argc, char **argv) {
  ...
  msg->Clear();
  msg->Write(STRIP_CSR, 0x00200000 + (chmask << 8)); //  Reset CSR - reset
                                                     //  trigger counter,
  ...
  msg->WaitClear(STRIP_BCO_DCM, 0x80000000); //  Wait DCM write to finish - BCO
                                             //  frequency is now 12.500 MHz.
  msg->Write(STRIP_CSR, 0x80200000 + (chmask << 8)); //  Reset the DCM
  msg->Write(STRIP_CSR, 0x00200000 + (chmask << 8)); //  Reset the DCM -
  ...
  msg->Write(STRIP_CSR, 0x0f000030 | (chmask << 8)); //  Turn on data readout in
                                                     //  CSR but don't veto
                                                     //  untriggered events.
