...
int cmd_tsclk(const struct s_cmd *c) {
  ...
  if (narg == 1) {
    ...
    _print_float(f);
    _puts(" MHz -");
    if (!(CAPTAN_VIPIC_CSR & 0x40000000))
      _puts(" not");
    _puts(" locked");
  } else if (narg > 1) {
    for (i = 1; i < narg; i++) {
...
struct s_cmd cmds[] = {{"peek", 1, cmd_peek},       {"poke", 2, cmd_poke},
                       ...
                       {"dhcp", 0, cmd_dhcp},       {"halt", 0, cmd_halt},
                       {"start", 0, cmd_start},     {"trig", 0, cmd_trig},
                       {"time", 0, cmd_time},       {"send", 2, cmd_send},
                       {"ts", 0, cmd_ts},           {"bco", 0, cmd_ts},
                       {"tsclk", 0, cmd_tsclk},     {"bdclk", 0, cmd_bdclk},
                       {"restart", 0, cmd_restart}, {"clean", 0, cmd_clean},
                       {"set", 0, cmd_set},         {"res", 0, cmd_res},
                       {"cal", 1, cmd_cal},         {"idelay", 1, cmd_idelay},
                       {"read", 0, cmd_read},       {"go", 0, cmd_go},
                       {"tsread", 0, cmd_tsread},   {"spy", 0, cmd_spy},
                       {"log", 0, cmd_log},         {"ver", 0, cmd_ver},
                       {"stat", 0, cmd_stat},       {"state", 0, cmd_state},
                       {"reset", 0, cmd_reset},     {"bye", NULL}};
