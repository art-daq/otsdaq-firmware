...
#define CAPTAN_STRIP_LINK_STATUS(i,j) (*(volatile unsigned int *)(0xc4000030+0x100*(j)+0x1000*(i)))
...
static unsigned char set_number[]   = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                        0xff, 0xff,    0,    1,    4,    5,    3,    2,
                                        0xff, 0xff,   12,   13,    8,    9,   11,   10,
                                        0xff, 0xff,   15,   14,    7,    6, 0xff, 0xff };
static unsigned char strip_number[] = { 0xff, 0xff, 0xff, 0xff,
                                        0xff,    0,    2,    1,
                                        0xff,    6,    4,    5,
                                        0xff,    7,    3, 0xff };
...
//      t = 25;       //  This is for a 40 MHz external clock
    ...
    _print_float(f);
    _puts(" MHz measured -");
    if ( ! (CAPTAN_STRIP_CSR&0x20000000) ) _puts(" not");
    _puts(" locked");
  }
  else if ( narg > 1 ) {
    for ( i=1; i<narg; i++ ) {
...
int cmd_dac(const struct s_cmd *c) {
  ...
  if ( narg == 4 ) {
    if ( _strcmp(args[1],"read") == 0 ) {
      ...
      addr = _atoh(args[3]);
      if ( addr < 0 || addr > 0x0b ) return 1;
      CAPTAN_DAC_SPI = 0x80008000 | (1<<(16+mask)) | (addr<<8);
      loop = 0;
      while ( (CAPTAN_DAC_SPI&0x80000000) ) loop += 1;
...
//
    ...
    if ( _strcmp(args[1],"write") == 0 ) {
      mask = _atoh(args[2]);
      if ( mask < 0 || mask > 16 ) return 1;
      addr = _atoh(args[3]);
      if ( addr < 0 || addr > 0x0b ) return 1;
      data = _atoh(args[4]);
      CAPTAN_DAC_SPI = 0x80000000 | (mask<<16) | ((addr&0x0f)<<8) | (data&0xff);
    }
  }
  return 0;
}
...
int cmd_link(const struct s_cmd *c) {
  ...
  static int ichan = -1;
//  static int ichip = -1;
  static const char *bits[8] = { "   Aligned",
                                 "Sync error",
                                 "  SendData",
                                 "RejectHits",
                                 "   Alines0",
  ...
  if ( narg == 2 ) {
    ...
    if ( _strcmp(args[1],"fifo") == 0 ) {
      if ( narg > 2 ) ichan = _atoi(args[2]);
      if ( ichan < 0 || ichan > 7 ) return 1;
      ...
      while ( ! (CAPTAN_STRIP_FIFO_CSR&(1<<ichan)) && ! (UART_SR&0x01) ) {
        ...
        _print_bits(w,3,1);
        _putch('=');
        _print_int(128*(((w>>24)&0x07)-1)+strip_number[(w>>17)&0x0f]+8*set_number[(w>>12)&0x1f]);
      }
      if ( (UART_SR&0x01) ) {
        _getch();
      }
  ...
  if ( narg == 1 ) {
    ...
    for ( i=1; i<narg; i++ ) {
      if ( _strcmp(args[i],"on") == 0 ) {
        ...
        if ( ichan < 0 ) {
          ichan = _atoi(args[i]);
          if ( ichan < 0 || ichan > 5 ) return 1;
        }
        else if ( ichip < 0 ) {
          ichip = _atoi(args[i]);
          if ( ichip < 1 || ichip > 5 ) return 1;
        }
      }
      else {
        return 1;
    ...
    if ( ion == 0 ) w &= 0x7fffffff;
    if ( ion == 1 ) w |= 0x80000000;
    if ( ichip >= 0 ) w = ( (w&0xffffff0f) | (ichip<<4) );
    if ( ichan >= 0 ) w = ( (w&0xfffffff0) | ichan );
...
struct s_cmd cmds[] = { { "peek", 1, cmd_peek },
                        ...
                        { "pctrl", 0x0203, cmd_sc },
                        { "vbn", 0x0303, cmd_sc },
                        { "shpvbp2", 0x0403, cmd_sc },
                        { "shpvbp1", 0x0503, cmd_sc },
                        { "blrvbp1", 0x0603, cmd_sc },
                        { "dvtn", 0x0703, cmd_sc },
                        { "dthr0", 0x0803, cmd_sc },
                        { "dthr1", 0x0903, cmd_sc },
                        { "dthr2", 0x0a03, cmd_sc },
                        ...
                        { "aline", 0x1002, cmd_sc },
                        { "kill", 0x1104, cmd_sc },
                        { "inject", 0x1204, cmd_sc },
                        { "sdata", 0x1301, cmd_sc },
                        { "reject", 0x1401, cmd_sc },
                        { "spr", 0x1800, cmd_sc },
                        { "dcr", 0x1b03, cmd_sc },
                        { "scr", 0x1c10, cmd_sc },
                        { "aqbco", 0x1e23, cmd_sc },
...
void handler(void) {
  ...
  static const char *cmdname[] = { "nop", "write  ", "set    ", "recv   ", "read   ", "reset  ", "default", "waitclr", "waitset" };
