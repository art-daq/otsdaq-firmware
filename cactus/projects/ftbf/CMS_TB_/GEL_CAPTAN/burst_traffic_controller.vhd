	...
	architecture burst_traffic_controller_arch of burst_traffic_controller is		  
		...
		signal clocks_since_write : std_logic_vector(17 downto 0); 
		...
		begin
			...
			if rising_edge(MASTER_CLK) then
				...
				if reset = '1' then
					...
					clocks_since_write <= (others => '0'); 		
					writes_in_curr_burst <= (others => '0'); 	   	
					
				else						-- main section	
					
					if clocks_since_write /= "10" & x"0000" then -- increment clocks since last burst write received
						clocks_since_write <= clocks_since_write + 1;	 
					end if;	 
					...
					if BURST_WE = '1' then 
						...
						if writes_in_curr_burst /= 0 and
							clocks_since_write >= '0' & '1' & x"E848" then--BURST_PERIOD_MAX & '1' & x"E848" then -- in ms : [<val> * 0x1E848] 125Mhz clocks is max wait
