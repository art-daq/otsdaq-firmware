...
-------------------------------------------------------------------------------
--
-- File        : c:\NIM_Plus\AGP_2016\NIMPlus\NIMPlus\compile\veto_count.vhd
-- Generated   : 03/28/17 12:17:47
-- From        : c:\NIM_Plus\AGP_2016\NIMPlus\NIMPlus\src\veto_count.asf
	...
	case Sreg0 is
		when S1 =>
			if sig_in = '1' and count_in /= z_32 then
				...
				NextState_Sreg0 <= S1;
				next_count_reg <= count_in;
				next_veto_out_n <= '1';
			end if;
		when S2 =>
			if count_reg /= z_32 then
				NextState_Sreg0 <= S2;
				next_count_reg <= count_reg - u_32;
				next_veto_out_n <= '0';
			elsif count_reg = z_32 then
				NextState_Sreg0 <= S1;
				next_count_reg <= count_in;
				next_veto_out_n <= '1';
			end if;
