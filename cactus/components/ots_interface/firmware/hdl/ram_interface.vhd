...
--erased for implementation
	...
	CONTROLLER : entity work.ram_controller
        ...
        write_fifo : RAM_WRITE_FIFO 					 
            port map(									 
                rst => reset,							 
                wr_clk => MASTER_CLK,					 
                rd_clk => FLASH_CLK,					 
                din => ram_write_data_in,				 
                wr_en => fifo_in_we,					 
                rd_en => fifo_in_re,					 
        ...
        read_fifo : RAM_READ_FIFO 						 
            port map(									 
                rst => reset,							 
                wr_clk => FLASH_CLK,					 
                rd_clk => MASTER_CLK,					 
                din => ram_read_data_in,				 
                wr_en => fifo_out_we,					 
                rd_en => fifo_out_re,					 
                dout => ram_read_data_out,				 
                full => open,							 
                empty => open); --fifo_out_empty);		 
