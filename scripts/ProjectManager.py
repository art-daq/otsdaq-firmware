...
class SvnPlugin(Plugin):
    ...
    def _findFirstExisting(self, svnroot, paths):
        ...
        for p in paths:
            try:
                self._checkPath(os.path.join(svnroot, p))
    ...
    def execute(self):
        ...
        if self.fromtag:
           ...
           # Discover the correct folder to check out
            ...
            svnpath = self._findFirstExisting( tagPath, [
            ...
            # Take the project from the same area as the core framework
            # Build the path to fetch projects/<proj>
            folders = [os.path.join('projects', self.project)]
...
class CactusCreator(SvnPlugin):
    ...
    def addArguments(subparsers,cmd):
        ...
        subp.add_argument('--board',             help='board area to check out.', default=None)
...
#------------------------------------------------------------------------------
#    __  ___        __    __    _       __                          ___                ___       _ __   __
#   /  |/  /__  ___/ /__ / /__ (_)_ _  / /  ___ ___ ____ _______ __/ _ | _______ ___ _/ _ )__ __(_) /__/ /__ ____
#  / /|_/ / _ \/ _  / -_) (_-</ /  ' \/ /__/ -_) _ `/ _ `/ __/ // / __ |/ __/ -_) _ `/ _  / // / / / _  / -_) __/
# /_/  /_/\___/\_,_/\__/_/___/_/_/_/_/____/\__/\_, /\_,_/\__/\_, /_/ |_/_/  \__/\_,_/____/\_,_/_/_/\_,_/\__/_/
#                                             /___/         /___/
#------------------------------------------------------------------------------
...
MODELSIM_ROOT=/opt/mentor/modeltech/

# Location of pre-compiled Xilinx libraries
MODELSIM_IN_XILINX="mti_se/10.1b"
ISE_VHDL_MTI="${XILINX}/vhdl/${MODELSIM_IN_XILINX}/lin64"
ISE_VLOG_MTI="${XILINX}/verilog/${MODELSIM_IN_XILINX}/lin64"

vars="MODELSIM_ROOT ISE_VHDL_MTI ISE_VLOG_MTI"
...
export MTI_VCO_MODE MODELSIM_ROOT ISE_VHDL_MTI ISE_VLOG_MTI
'''
...
class ModelsimAreaBuilder(WorkareaBuilder):
    ...
    def make(self,workarea,env):
        ...
        with open(mksim,'w') as f:
            f.write(
                mkSimLegacyTemplate.format(**env)
                )
            self._log.info('File %s created' % mksim)

#------------------------------------------------------------------------------
#    ____        ___                ___       _ __   __
#   /  _/__ ___ / _ | _______ ___ _/ _ )__ __(_) /__/ /__ ____
#  _/ /(_-</ -_) __ |/ __/ -_) _ `/ _  / // / / / _  / -_) __/
# /___/___/\__/_/ |_/_/  \__/\_,_/____/\_,_/_/_/\_,_/\__/_/
#
#------------------------------------------------------------------------------
...
$(PROJECTFILE):
\t@$(DEPTREE) -p x $(UPGRADES_ROOT) $(BUILD_PROJECT) $(DEPFILE) -o mkProject.tcl
...
#------------------------------------------------------------------------------
# IseAreaBuilder implementation
#------------------------------------------------------------------------------

class IseAreaBuilder(WorkareaBuilder):
    _log = logging.getLogger(__name__)

    def __init__(self, **kwargs):
        super(IseAreaBuilder,self).__init__(**kwargs)
...
DEPFILE:=$(TOPLVL).dep
PROJECTFILE:=$(TOPLVL)/$(TOPLVL).xpr
BITFILE:=$(TOPLVL)/$(TOPLVL).runs/impl_1/$(TOPLVL).bit
PACKAGEFILE:=$(PKGNAME)_$(TIMESTAMP).tgz
...
\tcp $(BITFILE) pkg/src/
\tcp -a addrtab/ pkg/src/
\ttar cvfz pkg/$(PACKAGEFILE) -C pkg/src addrtab $(TOPLVL).bit
